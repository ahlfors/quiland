//package creative.fire.tls.cert.java;
//
///*
...
//			doCommands(out);
//		} catch (Exception e) {
//			PConsole.print(rb.getString("keytool error: ") + e);
//			if (verbose) {
...
//				command = DELETE;
//			} else if (collator.compare(flags, "-export") == 0 || collator.compare(flags, "-exportcert") == 0) {
//				command = EXPORTCERT;
//			} else if (collator.compare(flags, "-genkey") == 0 || collator.compare(flags, "-genkeypair") == 0) {
//				command = GENKEYPAIR;
...
//				command = IDENTITYDB;
//			} else if (collator.compare(flags, "-import") == 0 || collator.compare(flags, "-importcert") == 0) {
//				command = IMPORTCERT;
...
//					}
//				}
//				providers.add(new Pair<String, String>(providerClass, providerArg));
//			}
//
//			/*
//			 * options
...
//			throw new UnsupportedOperationException(MessageFormat.format(rb.getString("-storepasswd and -keypasswd commands not supported "
...
//					MessageFormat.format(rb.getString("-keypass and -new " + "can not be specified if -storetype is {0}"), storetype));
//		}
//
//		if (protectedPath) {
//			if (storePass != null || keyPass != null || newPass != null || destKeyPass != null) {
//				throw new IllegalArgumentException(rb.getString("if -protected is specified, " + "then -storepass, -keypass, and -new "
...
//				throw new IllegalArgumentException(rb.getString("if -srcprotected is specified, " + "then -srcstorepass and -srckeypass "
//						+ "must not be specified"));
//			}
//		}
//
...
//			if (storePass != null || keyPass != null || newPass != null || destKeyPass != null) {
//				throw new IllegalArgumentException(rb.getString("if keystore is not password protected, " + "then -storepass, -keypass, and -new "
//						+ "must not be specified"));
//			}
//		}
//
...
//			if (srcstorePass != null || srckeyPass != null) {
//				throw new IllegalArgumentException(rb.getString("if source keystore is not password protected, " + "then -srcstorepass and -srckeypass "
//						+ "must not be specified"));
//			}
//		}
//
...
//					if (command != GENKEYPAIR && command != GENSECKEY && command != IDENTITYDB && command != IMPORTCERT && command != IMPORTKEYSTORE) {
...
//		 * 
//		 * At this point, it's OK if no keystore password has been provided. We want to make sure that we can load the keystore data, i.e., the keystore data
//		 * has the right format. If we cannot load the keystore, why bother asking the user for his or her password? Only if we were able to load the keystore,
//		 * and no keystore password has been provided, will we prompt the user for the keystore password to verify the keystore integrity. This means that the
//		 * keystore is loaded twice: first load operation checks the keystore format, second load operation verifies the keystore integrity.
//		 * 
//		 * If the keystore password has already been provided (at the command line), however, the keystore is loaded only once, and the keystore format and
//		 * integrity are checked "at the same time".
//		 * 
...
//					&& (command == CERTREQ || command == DELETE || command == GENKEYPAIR || command == GENSECKEY || command == IMPORTCERT
//							|| command == IMPORTKEYSTORE || command == KEYCLONE || command == CHANGEALIAS || command == SELFCERT || command == STOREPASSWD
...
//					return;
//				}
//			} else if (!protectedPath && !KeyStoreUtil.isWindowsKeyStore(storetype) && !(command == PRINTCERT)) {
...
//			MessageFormat form = new MessageFormat(rb.getString("Warning:  Different store and key passwords not supported "
...
//		if (command == PRINTCERT || command == IMPORTCERT || command == IDENTITYDB) {
...
//				MessageFormat form = new MessageFormat(rb.getString("Alias <alias> references an entry type that is not a private key entry.  "
...
//	 * 
//	 * @param alias
//	 *            the entry we will set password for
//	 * @param orig
//	 *            the original entry of doing a dup, null if generate new
//	 * @param origPass
//	 *            the password to copy from if user press ENTER
//	 */
...
//
//		if (verbose) {
//			MessageFormat form = new MessageFormat(rb.getString("Generating keysize bit keyAlgName key pair and self-signed certificate "
//					+ "(sigAlgName) with a validity of validality days\n\tfor: x500Name"));
//			Object[] source = { new Integer(keysize), privKey.getAlgorithm(), chain[0].getSigAlgName(), new Long(validity), x500Name };
//			System.err.println(form.format(source));
//		}
//
//		if (keyPass == null) {
...
//
//	/**
//	 * Imports a JDK 1.1-style identity database. We can only store one certificate per identity, because we use the identity's name as the alias (which
//	 * references a keystore entry), and aliases must be unique.
//	 */
...
//		for (Enumeration<Identity> enum_ = idb.identities(); enum_.hasMoreElements();) {
//			Identity id = enum_.nextElement();
//			newCert = null;
//			// only store trusted identities in keystore
//			if ((id instanceof SystemSigner && ((SystemSigner) id).isTrusted()) || (id instanceof SystemIdentity && ((SystemIdentity) id).isTrusted())) {
...
//						keyStore.setKeyEntry(id.getName(), privKey, storePass, chain);
//					} else {
...
//					out.println(rb.getString("Certificate fingerprint (MD5): ") + getCertFingerPrint("MD5", chain[0]));
//				}
//			}
...
//				out.println(rb.getString("Certificate fingerprint (MD5): ") + getCertFingerPrint("MD5", cert));
//			}
//		} else {
...
//		}
//		/*
//		 * Information display rule of -importkeystore 1. inside single, shows failure 2. inside all, shows sucess 3. inside all where there is a failure,
//		 * prompt for continue 4. at the final of all, shows summary
//		 */
//	}
//
//	/**
...
//					rb.getString("Problem importing entry for alias <alias>: <exception>.\nEntry for alias <alias> not imported."));
...
//
//		MessageFormat form;
//		form = (keyStore.size() == 1) ? new MessageFormat(rb.getString("Your keystore contains keyStore.size() entry")) : new MessageFormat(
//				rb.getString("Your keystore contains keyStore.size() entries"));
...
//
//	/**
//	 * Reads a certificate (or certificate chain) and prints its contents in a human readbable format.
//	 */
...
//	 * 
//	 * <p>
//	 * Builds a certificate chain on top of the certificate reply, using trusted certificates from the keystore. The chain is complete after a self-signed
//	 * certificate has been encountered. The self-signed certificate is considered a root certificate authority, and is stored at the end of the chain.
//	 * 
//	 * <p>
...
//	 * 
//	 * @param prompt
//	 *            the message that gets prompted on the screen
//	 * @param oldPasswd
//	 *            the current (i.e., old) password
//	 */
...
//	 * 
//	 * @param prompt
//	 *            the {0} of "Enter {0} alias name:  " in prompt line
...
//		Object[] source = { cert.getSubjectDN().toString(), cert.getIssuerDN().toString(), cert.getSerialNumber().toString(16), cert.getNotBefore().toString(),
//				cert.getNotAfter().toString(), getCertFingerPrint("MD5", cert), getCertFingerPrint("SHA1", cert), cert.getSigAlgName(), cert.getVersion() };
//		out.println(form.format(source));
//
//		int extnum = 0;
...
//		}
//		if (trustcacerts && (caks != null) && (caks.getCertificateAlias(cert) != null)) {
...
//	 * 
//	 * @return an array of objects, where the 1st element in the array is the recovered private key, and the 2nd element is the password used to recover it.
//	 */
...
//		}
//
//		return new Object[] { (Key) key, keyPass };
//	}
//
//	/**
//	 * Recovers entry associated with given alias.
//	 * 
//	 * @return an array of objects, where the 1st element in the array is the recovered entry, and the 2nd element is the password used to recover it (null if
//	 *         no password).
//	 */
...
//
//	/**
//	 * Validates chain in certification reply, and returns the ordered elements of the chain (with user certificate first, and root certificate last in the
//	 * array).
//	 * 
//	 * @param alias
//	 *            the alias name
//	 * @param userCert
//	 *            the user certificate of the alias
//	 * @param replyCerts
//	 *            the chain provided in the reply
//	 */
...
//				for (Enumeration<String> aliases = caks.aliases(); aliases.hasMoreElements();) {
...
//
//	/**
//	 * Establishes a certificate chain (using trusted certificates in the keystore), starting with the user certificate and ending at a self-signed certificate
...
//
//	/**
//	 * Stores the (leaf) certificates of a keystore in a hashtable. All certs belonging to the same CA are stored in a vector that in turn is stored in the
//	 * hashtable, keyed by the CA's subject DN
//	 */
